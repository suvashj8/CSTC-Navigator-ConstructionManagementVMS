/**
 * Prepare local dev stack: wait for host Postgres, start Redis in Docker, seed demo accounts.
 */
...
for (let attempt = 1; attempt <= 2; attempt++) {
  try {
    ...
    if (attempt === 2) {
      console.warn("\nRedis container failed to start — continuing without Docker Redis.");
    } else {
